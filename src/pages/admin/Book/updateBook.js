...
const updateBook = () => {
    ...
    useEffect(() => {
        // console.log(params)
        axios.get(`http://localhost:5000/api/books/${params.id}`)
        .then((res)=>{setCurrentBook(res.data.book)})
        axios.get('http://localhost:5000/api/categories').then((res)=>setCategories(res.data.categories)).catch((err)=>console.log(err))
        axios.get('http://localhost:5000/api/writers').then((res)=>setWriters(res.data.writers)).catch((err)=>console.log(err))
    ...
    const initialValues = {
      ...
      const onSubmit = values =>{
       ...
       axios.post('http://localhost:5000/api/books', formik.values )
    ...
    //   if(Object.values(dataErr).every((value) => value == '')){
     ...
     axios.put(`http://localhost:5000/api/books/${currentBook._id}`,data).then((r)=>{console.log(r) }).catch((err)=>{console.log(err)})
        ...
        <label htmlFor="category" className="form-label">Select Category : </label>
              <select  onChange={(e)=>onInputChange(e)} id="category" name="category" class="form-select" aria-label="Select Category">
                ...
                {categories ? categories.map((category)=>{
                  return(
                    category._id === currentBook.category._id ? 
      ...
      <label htmlFor="category" className="form-label">Select Writer : </label>
              <select  onChange={(e)=>onInputChange(e)} id="writer" name="writer" class="form-select" aria-label="Select Writer">
                ...
                {writers !== null ? writers.map((writer)=>{
                  {console.log(currentBook.writer)}
                  return(
                    
                    writer._id === currentBook.writer ? 
